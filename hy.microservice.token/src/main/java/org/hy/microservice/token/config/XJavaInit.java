package org.hy.microservice.token.config;
...
public class XJavaInit extends AppInitConfig
{
    ...
    private synchronized void init(boolean i_IsStartJobs)
    {
        if ( !$Init )
        {
            ...
            try
            {
                this.init("config/token/ms.token.sys.Config.xml"     ,this.xmlRoot);
                this.init("config/token/ms.token.startup.Config.xml" ,this.xmlRoot);
                this.init((List<Param>)XJava.getObject("StartupConfig_MS_Token") ,this.xmlRoot);
                this.init(((Param)XJava.getObject("MS_Token_RootPackageName")).getValue());
            }
